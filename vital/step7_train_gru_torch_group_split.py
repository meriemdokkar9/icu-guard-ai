...
# PATHS
# =========================
X_PATH = "X_seq_24h.npy"          # (N, 24, F)
Y_PATH = "y_seq_24h.npy"          # (N,)
G_PATH = "patient_seq_24h.npy"    # (N,)
...
def standardize_by_train(X_tr, X_te):
    # X: (N,T,F) -> normalize per-feature using train stats across all timesteps
...
def main():
    ...
    model = GRUNet(in_dim=F, hidden=HIDDEN, layers=LAYERS, dropout=DROPOUT).to(device)
    ...
    # Save model
    torch.save(best_state, "gru_24h_model.pt")
    print("\nSaved: gru_24h_model.pt")
